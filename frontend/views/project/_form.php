...
<div class="project-create">
	    ...
	    <?= $form->field($model, 'projectmanager_id')->dropDownList(ArrayHelper::map($users, 'id', 'username'), [Yii::t('app','Select a projectmanager')])  ?>
	    
    <div id="existing-user" style="display:none;">
    	<fieldset>
    		<legend><?= Yii::t('app','Existing user') ?></legend>
	    		<?= Html::button('new User', ['onClick' => 'js:newContact()', 'class' => 'btn btn-default']) ?>
	    		
	    		<br>
    			
    			<?= $form->field($model, 'client_id')->dropDownList(ArrayHelper::map($users, 'id', 'username'), ['prompt' => Yii::t('app','Select a client')]) ?>
    	</fieldset>
    </div>
